...
public:
  ...
  // do this again
  //  METRICS UPGRADE: completely rewritten with all 7 improvements
  bool printPerformanceMetrics(float tickTime, float averageRush) {
    ...
    if (outfile.is_open()) {
      ...
      outfile << "Tick: " << (int)tickTime
              << endl; // METRICS UPGRADE #1: tick stamp
      ...
      if (this->totalArrivedCount > 0) {
        ...
        // METRICS UPGRADE #3: windowed average
      ...
      // METRICS UPGRADE #5: per-road congestion snapshot
